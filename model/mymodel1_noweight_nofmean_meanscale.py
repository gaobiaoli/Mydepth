# 原始拼接描述符模型：使用 CLS+patch 均值预测尺度，保留原生 DAv2 F36 refiner 路径。
...
from loss.common import masked_downsample, masked_frame_mean
...
@torch.no_grad()
def mean_log_scale(da3_depth, gt_depth, gt_valid, min_pixels=100):
    """Per-frame mean log ratio, leaving a naturally zero-mean residual."""
    valid = (
        (gt_valid > 0)
        & (da3_depth > 0)
        & (gt_depth > 0)
    ...
    valid_pixels = valid.flatten(1).sum(1)
    supported = valid_pixels >= min_pixels
    log_ratio = torch.where(
        valid,
        gt_depth.clamp_min(1e-6).log()
        - da3_depth.clamp_min(1e-6).log(),
        torch.zeros_like(gt_depth),
    )
    log_scale = log_ratio.flatten(1).sum(1)
    log_scale = log_scale / valid_pixels.clamp_min(1)
    supported &= torch.isfinite(log_scale)
...
class PriorBIMDA(nn.Module):
    ...
    def load_checkpoint(self, path: str | Path):
        ...
        # Some deterministic experiments pre-resized the trainable DINOv2
        # position grid from 37x37 to 36x36 before saving.  Recreate the
        # Parameter with the checkpoint's shape so those otherwise-compatible
        # models can still be evaluated with strict state loading.
        position_key = "dav2.backbone.embeddings.position_embeddings"
        if position_key in state:
            saved_position = state[position_key]
            embeddings = self.dav2.backbone.embeddings
            current_position = embeddings.position_embeddings
            if saved_position.shape != current_position.shape:
                if (
                    saved_position.ndim != 3
                    or saved_position.shape[0] != 1
                    or saved_position.shape[2] != current_position.shape[2]
                ):
                    raise ValueError(
                        "Checkpoint contains an incompatible DINOv2 position "
                        f"embedding shape: {tuple(saved_position.shape)}"
                    )
                embeddings.position_embeddings = nn.Parameter(
                    current_position.new_empty(saved_position.shape),
                    requires_grad=current_position.requires_grad,
                )
