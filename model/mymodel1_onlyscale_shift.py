...
class PriorBIMDA(nn.Module):
    """DAv2 metric depth with a learned global scale."""
    ...
    def forward(self, rgb, da3_depth, bim_depth, bim_valid):
        ...
        metric_shift = (
            reference_depth * relative_shift
        )

        affine_depth = (
            raw_depth * log_scale.exp()
            + metric_shift
        )
        ...
        return {
            ...
            "scaled_depth": raw_depth * log_scale.exp(),
            "log_scale": log_scale,
    ...
    def parameter_groups(self,factor=1.0):
        """Optimize only the backbone, BIM conditioning, and scale head."""
        return [
